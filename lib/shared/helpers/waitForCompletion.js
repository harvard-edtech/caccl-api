...
var waitForCompletion = function (opts) { return __awaiter(void 0, void 0, void 0, function () {
    var timeoutMs, stopTime, urlNoQuery, hostAndPath, checkPath, refreshMs, maxNumChecks, i, statusResponse;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                timeoutMs = (60000 * (opts.timeoutMin || 2));
                stopTime = (Date.now() + timeoutMs);
                urlNoQuery = String(opts.progress.url).split('?')[0].split('#')[0];
                hostAndPath = (urlNoQuery
                    // Remove protocol
                    .replace('http://', '')
                    .replace('https://', ''));
                checkPath = (hostAndPath.substring(hostAndPath.split('/')[0].length, hostAndPath.length));
